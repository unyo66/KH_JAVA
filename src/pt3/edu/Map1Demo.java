...
//		s3.map(s -> s.substring(1)).mapToInt(Integer::parseInt).mapToObj(i -> "b" + i).forEach(Util::print);
//		s3.map(s -> s.replace('a', 'b')).forEach(Util::print);
		s3.map(s -> "b" + s.substring(1)).forEach(Util::print);
	}
